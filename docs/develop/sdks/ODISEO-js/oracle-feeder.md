...
The following code segment demonstrates how to create a simple price oracle feeder with ODISEO.js by using a [LocalODISEO](https://github.com/ODISEOmoney/LocalODISEO) network. The prices aren't live in this example. It just tracks the public LCD node's current registered oracle price and submits them to the local testnet.

Note the following:

1. Prevotes are automatically generated using [`MsgAggregateExchangeRateVote.getPrevote()`](https://ODISEOmoney.github.io/ODISEO.js/classes/msgexchangeratevote.html#getprevote)
...
const {
  MAINNET_LCD_URL = "https://lcd.ODISEO.dev",
  MAINNET_CHAIN_ID = "columbus-5",
  TESTNET_LCD_URL = "http://localhost:1317",
  TESTNET_CHAIN_ID = "localODISEO",
